...
from yolov5_Distance_Detect.estimate_distance import DistanceEstimation
...
if __name__ == '__main__':
    ...
    inside_points=[]
    scale_factor = 1.1
    lines = DE.Detect(weights="weights/yolov5l.pt",source="point\\000008\\000008.png",view_img=True)
    lines_np = []
    for item in lines:
        # 将张量从 GPU 转移到 CPU 并转换为 numpy 数组
        tensor_item_1 = item[0].cpu().numpy()
        tensor_item_6 = item[5].cpu().numpy()
        
        # 保留其他的浮点数
        float_items = item[1:5] + (tensor_item_6,) + item[6:]
        
        # 将转换后的张量 numpy 数组和其他数据合并
        numpy_item = (tensor_item_1,) + float_items
        
        lines_np.append(numpy_item)
    for line in lines_np:
        category, u_center, v_center, width, height, conf,distance = line
        print(line)
